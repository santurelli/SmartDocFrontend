...
const ScelteArticoloManagementModal = ({ onClose }) => {
    ...
    const loadData = async () => {
        setLoading(true);
        try {
            const res = await ArticoliService.getListScelte({ descrizione: searchTerm });
            ...
            console.error("Error loading scelte:", error);
            Swal.fire({
                title: 'Errore',
                text: 'Impossibile caricare le scelte',
                icon: 'error',
    ...
    const handleDelete = async (id) => {
        const result = await Swal.fire({
            title: 'Sei sicuro?',
            text: "La scelta verrà eliminata.",
            icon: 'warning',
        ...
        if (result.isConfirmed) {
            try {
                const res = await ArticoliService.deleteScelta(id);
    ...
    const openDetailModal = (id, existingDesc) => {
        Swal.fire({
            title: id ? 'Modifica Scelta' : 'Nuova Scelta',
            html: `
                <div style="text-align: left; padding: 10px 5px;">
                    <div class="form-group">
                        <label class="premium-swal-label">Descrizione Scelta</label>
                        <input id="swal-descrizione" class="form-control premium-swal-input" placeholder="Es. Prima Scelta, Commerciale..." value="${existingDesc}">
                    </div>
                </div>
            `,
            showCancelButton: true,
            ...
            preConfirm: async () => {
                ...
                try {
                    let res;
                    if (id) {
                        res = await ArticoliService.updateScelta(id, { descrizione: value });
                    } else {
                        res = await ArticoliService.createScelta({ descrizione: value });
                    }
    ...
    const filteredList = sortedList; // Search is done on backend on call loadData if needed, or here
            ...
            <div className="modal-dialog modal-lg" role="document">
                <div className="modal-content">
                    <div className="modal-header bg-primary" style={{ color: 'white', borderTopLeftRadius: '6px', borderTopRightRadius: '6px' }}>
                        ...
                        <h4 className="modal-title" style={{ fontWeight: 'bold' }}>Elenco Scelte Articolo</h4>
                    </div>
...
export default ScelteArticoloManagementModal;
