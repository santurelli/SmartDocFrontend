...
const ToniArticoloManagementModal = ({ onClose }) => {
    ...
    const loadData = async () => {
        setLoading(true);
        try {
            const res = await ArticoliService.getListToni({ descrizione: searchTerm });
            ...
            console.error("Error loading toni:", error);
            Swal.fire({
                title: 'Errore',
                text: 'Impossibile caricare i toni',
                icon: 'error',
    ...
    const handleDelete = async (id) => {
        ...
        if (result.isConfirmed) {
            try {
                const res = await ArticoliService.deleteTono(id);
    ...
    const openDetailModal = (id, existingDesc) => {
        Swal.fire({
            title: id ? 'Modifica Tono' : 'Nuovo Tono',
            html: `
                <div style="text-align: left; padding: 10px 5px;">
                    <div class="form-group">
                        <label class="premium-swal-label">Descrizione Tono</label>
                        <input id="swal-descrizione" class="form-control premium-swal-input" placeholder="Es. Tono A, Tono B..." value="${existingDesc}">
                    </div>
                </div>
            `,
            showCancelButton: true,
            ...
            preConfirm: async () => {
                ...
                try {
                    let res;
                    if (id) {
                        res = await ArticoliService.updateTono(id, { descrizione: value });
                    } else {
                        res = await ArticoliService.createTono({ descrizione: value });
                    }
            ...
            <div className="modal-dialog modal-lg" role="document">
                <div className="modal-content">
                    <div className="modal-header bg-primary" style={{ color: 'white', borderTopLeftRadius: '6px', borderTopRightRadius: '6px' }}>
                        ...
                        <h4 className="modal-title" style={{ fontWeight: 'bold' }}>Elenco Toni Articolo</h4>
                    </div>
...
export default ToniArticoloManagementModal;
