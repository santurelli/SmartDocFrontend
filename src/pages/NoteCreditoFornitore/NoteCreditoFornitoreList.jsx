...
const NoteCreditoFornitoreList = () => {
    const navigate = useNavigate();

    const [loading, setLoading] = useState(true);
    const [note, setNote] = useState([]);
    const [total, setTotal] = useState(0);
    const [totali, setTotali] = useState({ f: 0, s: 0, d: 0 }); // f = totale, s = rimborsato, d = da rimborsare
    ...
    const fetchNote = async () => {
        ...
        try {
            ...
            const res = await NoteCreditoFornitoreService.getList(params);
            const payload = res.data?.payload;
            if (payload) {
                setNote(payload.list || []);
            ...
            console.error('Errore nel caricamento delle note di credito:', error);
            Swal.fire('Errore', 'Impossibile caricare le note di credito', 'error');
            setNote([]);
            setTotal(0);
        } finally {
            setLoading(false);
    ...
    const handleReset = () => {
        ...
        setLoading(true);
        NoteCreditoFornitoreService.getList({
            start: 0,
            length: pageSize,
            orderColumn: 0,
            orderDir: 'desc'
    ...
    const handleDelete = async (id) => {
        const result = await Swal.fire({
            title: 'Sei sicuro?',
            text: "Non potrai recuperare questa nota di credito!",
            icon: 'warning',
        ...
        if (result.isConfirmed) {
            try {
                await NoteCreditoFornitoreService.delete(id);
                Swal.fire('Eliminata!', 'La nota di credito è stata eliminata.', 'success');
                fetchNote();
            } catch (error) {
                console.error('Errore durante l\'eliminazione:', error);
                Swal.fire('Errore', 'Impossibile eliminare la nota di credito', 'error');
            }
        }
    };
            ...
            <div className="toolbar-row">
                ...
                <div className="toolbar-right">
                    <button 
                        ...
                        onClick={() => navigate('/note-credito-fornitore/new')}
                    >
                        <FaPlus /> Nuova Nota Credito Fornitore
                    </button>
                </div>
            </div>
                        ...
                        <table className="table table-hover">
                            ...
                            <tbody>
                                {loading ? (
                                    ...
                                    <tr>
                                        <td colSpan="8" className="text-center">Nessuna nota di credito trovata.</td>
                                    </tr>
                                ) : (
                                    note.map(item => (
                                        <tr key={item.id}>
                                            ...
                                            <td>{item.ragioneSocialeFornitore || item.descFornitore || 'N/D'}</td>
                                                ...
                                                <div className={`action-menu-container${activeActionMenu === item.id ? ' is-open' : ''}`} style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', whiteSpace: 'nowrap' }}>
                                                    ...
                                                    {activeActionMenu === item.id && (
                                                        <div className="action-dropdown-menu">
                                                            <button className="action-dropdown-item" onClick={() => navigate(`/note-credito-fornitore/${item.id}`)}>
                                                                <FaEdit /> Modifica
                                                            </button>
...
export default NoteCreditoFornitoreList;
