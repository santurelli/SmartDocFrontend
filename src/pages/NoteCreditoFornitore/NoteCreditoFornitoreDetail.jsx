...
import { FaInfoCircle, FaCalendarAlt, FaFileAlt, FaAngleRight, FaSave, FaPlus, FaTrash, FaArrowLeft } from 'react-icons/fa';
import Swal from 'sweetalert2';
import Select from 'react-select';
import AsyncSelect from 'react-select/async';
import NoteCreditoFornitoreService from '../../services/NoteCreditoFornitoreService';
...
const NoteCreditoFornitoreDetail = () => {
    const { id } = useParams();
    ...
    const fetchNextNum = async (dateStr) => {
        ...
        try {
            ...
            const res = await NoteCreditoFornitoreService.getNextNum(formattedDate);
    ...
    // Fetch initial data (combos & credit note details)
    useEffect(() => {
        const fetchInitialData = async () => {
            try {
                const comboRes = await NoteCreditoFornitoreService.getCombosMap();
                let loadedCombos = {
                    aliquoteIva: [],
                    unitaMisura: [],
                    tipiPagamento: [],
                ...
                if (!isNew) {
                    const res = await NoteCreditoFornitoreService.getById(id);
                    ...
                    if (data) {
                        ...
                        // Map internal fields for consistency
                        data.descFornitore = data.ragioneSocialeFornitore || data.descFornitore || '';
                        data.numDocumento = data.numeroRegistrazione || data.numDocumento || '';
                        data.dataDocumento = data.dtRegistrazione || data.dataDocumento || '';
                        data.numeroDocumentoFornitore = data.numeroDocumento || data.numeroDocumentoFornitore || '';
                        data.dataDocumentoFornitore = data.dtDocumento || data.dataDocumentoFornitore || '';
                        data.totale = data.totaleDocumento || data.totale || 0;
                        data.totaleDaPagare = data.totaleDaSaldare || data.totaleDaPagare || 0;
                        setFormData(data);
                ...
                Swal.fire('Errore', 'Impossibile caricare i dati della nota di credito', 'error');
            } finally {
                setLoading(false);
            }
        };
    ...
    const validate = () => {
        ...
        if (!formData.numeroDocumentoFornitore) {
            Swal.fire('Attenzione', 'Inserire il numero della nota di credito del fornitore', 'warning');
            return false;
        }
        if (!formData.dataDocumentoFornitore) {
            Swal.fire('Attenzione', 'Inserire la data del documento del fornitore', 'warning');
            return false;
        }
    ...
    const handleSave = async (e) => {
        ...
        const payload = {
            ...
            // Re-map fields to fit NotaCreditoFornitoreDto expectations
            numeroRegistrazione: parseInt(formData.numDocumento) || 0,
            dtRegistrazione: formData.dataDocumento && formData.dataDocumento.includes('-')
                ? formData.dataDocumento.split('-').reverse().join('/')
                : formData.dataDocumento,
            numeroDocumento: formData.numeroDocumentoFornitore,
            dtDocumento: formData.dataDocumentoFornitore && formData.dataDocumentoFornitore.includes('-')
        ...
        try {
            await NoteCreditoFornitoreService.save(payload);
            Swal.fire({
                title: 'Salvato!',
                text: 'Nota di credito fornitore salvata con successo',
                icon: 'success',
                timer: 1500,
                showConfirmButton: false
            }).then(() => {
                navigate('/note-credito-fornitore');
            });
        } catch (error) {
            console.error('Errore durante il salvataggio:', error);
            const msg = error.response?.data?.message || error.response?.data || 'Impossibile salvare la nota di credito';
    ...
    if (loading) {
        return (
            <div className="fatture-detail-container text-center" style={{ paddingTop: '100px' }}>
                ...
                <p style={{ marginTop: '15px', color: '#888' }}>Caricamento in corso...</p>
            </div>
        );
    }
        ...
        <div className="fatture-detail-container entity-form-shared">
            ...
            <div id="fatture-content-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px', flexWrap: 'wrap', gap: '20px' }}>
                <div className="header-left">
                    <h1 style={{ color: '#03a9f4', fontWeight: 300, fontSize: '28px', margin: 0 }}>
                        {isNew ? 'Nuova Nota Credito Fornitore' : `Nota Credito Fornitore N. ${formData.numeroDocumentoFornitore}`}
                    </h1>
                    <div className="breadcrumb" style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12px', color: '#888', marginTop: '5px', textTransform: 'uppercase' }}>
                        <span style={{ cursor: 'pointer' }} onClick={() => navigate('/note-credito-fornitore')}>Elenco</span>
                        <FaAngleRight />
                        <span className="active" style={{ color: '#333', fontWeight: 600 }}>
                            {isNew ? 'Nuova' : `Nota Credito ${formData.numeroDocumentoFornitore}`}
                        </span>
                    </div>
                </div>
            ...
            <div className="detail-box main-box" style={{ background: '#fff', borderRadius: '8px', border: '1px solid #e7ebee', marginTop: '20px' }}>
                ...
                <div className="main-box-body" style={{ padding: '20px' }}>
                    <form className="tab-content" onSubmit={handleSave} autoComplete="off" noValidate>
                        {/* Tab Generale */}
                        <div className={`tab-pane ${activeTab === 'generale' ? 'active' : ''}`}>
                            <div className="row" style={{ alignItems: 'flex-start' }}>
                                ...
                                <div className="col-md-6" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                                    ...
                                    <div className="premium-card address-card" style={{ border: '1px solid #e7ebee', borderRadius: '8px' }}>
                                        <div className="card-header-vibrant" style={{ background: '#f8f9fa', padding: '12px 18px', borderBottom: '1px solid #f1f2f7', fontWeight: 700, fontSize: '13px', color: '#555', borderTopLeftRadius: '8px', borderTopRightRadius: '8px' }}>
                                            <FaFileAlt style={{ color: '#03a9f4', marginRight: '8px' }} /> Dati Nota Credito Fornitore
                                        </div>
                                        <div className="card-body" style={{ padding: '20px' }}>
                                            <div style={{ display: 'flex', gap: '15px', marginBottom: '15px' }}>
                                                <div style={{ flex: '1' }}>
                                                    <label className="premium-label" style={{ fontWeight: 600, color: '#666', fontSize: '12px', display: 'block', marginBottom: '5px' }}>N. Nota Credito Fornitore</label>
                                                    <input
                                                        type="text"
                                                        className="form-control premium-input"
                                                ...
                                                <div style={{ flex: '1' }}>
                                                    <label className="premium-label" style={{ fontWeight: 600, color: '#666', fontSize: '12px', display: 'block', marginBottom: '5px' }}>Data Nota Credito Fornitore</label>
                                                    <input
                                                        type="date"
                                                        className="form-control premium-input"
                            ...
                            <div style={{ padding: '20px' }}>
                                ...
                                <ScadenzeTable
                                    ...
                                    onRefreshConti={async () => {
                                        const res = await NoteCreditoFornitoreService.getCombosMap();
                                        if (res.data?.payload) {
                                            setCombos(prev => ({
                                                ...prev,
                            ...
                            <div className="premium-card" style={{ border: '1px solid #e7ebee', borderRadius: '8px' }}>
                                ...
                                <div className="card-body" style={{ padding: '20px' }}>
                                    <textarea
                                        ...
                                        onChange={handleHeaderChange}
                                        disabled={isReadOnly}
                                        placeholder="Note, dettagli aggiuntivi o informazioni utili per questa nota di credito..."
                        ...
                        <div className="form-actions" style={{ marginTop: '30px' }}>
                            <button type="button" className="btn btn-premium-cancel" onClick={() => navigate('/note-credito-fornitore')}>
                                <FaArrowLeft style={{ marginRight: '8px' }} /> Torna all'elenco
                            </button>
                            {!isReadOnly && (
                                <button type="submit" className="btn btn-premium-save">
                                    <FaSave style={{ marginRight: '8px' }} /> Salva Nota Credito
                                </button>
                            )}
                        </div>
                    </form>
...
export default NoteCreditoFornitoreDetail;
