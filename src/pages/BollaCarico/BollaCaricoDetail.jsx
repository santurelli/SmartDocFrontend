...
import { FaSave, FaArrowLeft, FaCaretDown, FaHome, FaTruck, FaGlobe, FaFileInvoiceDollar, FaBoxOpen, FaChevronDown, FaChevronUp } from 'react-icons/fa';
...
import './BollaCaricoDetail.css';
import '../../components/EntityForms.css';

const BollaCaricoDetail = () => {
    const { id } = useParams();
    ...
    const [ordiniAperti, setOrdiniAperti] = useState([]);
    const [showOrdiniAperti, setShowOrdiniAperti] = useState(true);
    ...
    useEffect(() => {
        ...
        if (!isNew) {
            ...
            fetchDataFromOrdine(fromOrdineId);
        } else {
    ...
    useEffect(() => {
        if (formData.idFornitore) {
            OrdiniService.getOrdiniAperti(formData.idFornitore).then(res => {
                const list = res.data?.payload || res.data || [];
                setOrdiniAperti(Array.isArray(list) ? list : []);
            }).catch(err => {
                console.error(err);
                setOrdiniAperti([]);
            });
    ...
    const fetchCombos = async () => {
        try {
            const res = await BollaCaricoService.getCombosMap();
    ...
    const fetchData = async () => {
        try {
            const res = await BollaCaricoService.getById(id);
            ...
            if (data) {
                ...
                if (data.dataDocumentoFornitore && data.dataDocumentoFornitore.includes('/')) {
                    const fParts = data.dataDocumentoFornitore.split('/');
                    data.dataDocumentoFornitore = `${fParts[2]}-${fParts[1]}-${fParts[0]}`;
                }
                ...
                setFormData(prev => ({ ...prev, ...data, idOrdine: data.idOrdine || [] }));
                const mappedProdotti = (data.prodotti || []).map(p => ({
                    ...p,
                    tipo: p.tipo || (p.idProdotto ? 'A' : (p.fmDescrizione ? 'F' : 'N')),
                    scarica: p.scarica === 0 ? 0 : 1
                }));
                setProdotti(mappedProdotti);
            }
        } catch (error) {
            console.error(error);
            Swal.fire('Errore', 'Impossibile caricare la bolla di carico', 'error');
            navigate('/bollecarico');
        }
    };
        ...
        try {
            ...
            if (data) {
                ...
                setFormData(prev => ({
                    ...
                    idNsBanca: data.idNsBanca,
                    indirizzoIntestazione: data.indirizzoIntestazione,
                    cittaIntestazione: data.cittaIntestazione,
                    capIntestazione: data.capIntestazione,
                    provinciaIntestazione: data.provinciaIntestazione,
                    nazioneIntestazione: data.nazioneIntestazione || 'Italia',
                    partitaIva: data.partitaIva,
                    codiceFiscale: data.codiceFiscale,
                    indirizzoDestinazione: data.indirizzoDestinazione,
                    cittaDestinazione: data.cittaDestinazione,
                    capDestinazione: data.capDestinazione,
                    provinciaDestinazione: data.provinciaDestinazione,
                    nazioneDestinazione: data.nazioneDestinazione || 'Italia',
                    idOrdine: [parseInt(ordineId)]
                }));
                fetchNextNum(formData.dataDocumento);
            }
        } catch (error) {
            console.error(error);
            Swal.fire('Errore', "Errore nel caricamento dati dall'ordine", 'error');
        }
    };
    ...
    const fetchNextNum = async (dateStr) => {
        ...
        try {
            ...
            const res = await BollaCaricoService.getNextNum(formattedDate);
    ...
    useEffect(() => {
        ...
        } else if (formData.idFornitore && isNew && !fromOrdineId) {
    ...
    const applyFornitoreAddress = (f) => {
        ...
        setFormData(prev => ({
            ...prev,
            indirizzoIntestazione: prev.indirizzoIntestazione || addr.indirizzo,
            cittaIntestazione: prev.cittaIntestazione || addr.citta,
            capIntestazione: prev.capIntestazione || addr.cap,
            provinciaIntestazione: prev.provinciaIntestazione || addr.provincia,
            nazioneIntestazione: prev.nazioneIntestazione || addr.nazione,
            indirizzoDestinazione: prev.indirizzoDestinazione || addr.indirizzo,
            cittaDestinazione: prev.cittaDestinazione || addr.citta,
            capDestinazione: prev.capDestinazione || addr.cap,
            provinciaDestinazione: prev.provinciaDestinazione || addr.provincia,
            nazioneDestinazione: prev.nazioneDestinazione || addr.nazione,
            partitaIva: f.partitaIva || prev.partitaIva || '',
            codiceFiscale: f.codiceFiscale || prev.codiceFiscale || '',
            idTipoPagamento: prev.idTipoPagamento || f.idTipoPagamento
        }));
    };
    ...
    const isOrdineLinked = (ordineId) => (formData.idOrdine || []).includes(ordineId);

    const toggleOrdineLink = (ordine) => {
        const linked = isOrdineLinked(ordine.id);
        if (linked) {
            setFormData(prev => ({ ...prev, idOrdine: (prev.idOrdine || []).filter(oid => oid !== ordine.id) }));
        } else {
            setFormData(prev => ({ ...prev, idOrdine: [...(prev.idOrdine || []), ordine.id] }));
            // Pull rows from this ordine too
            OrdiniService.getById(ordine.id).then(res => {
                const data = res.data?.payload || res.data;
                const mappedProdotti = (data.prodotti || []).map(p => ({
                    ...p,
                    id: 0,
                ...
                setProdotti(prev => [...prev, ...mappedProdotti]);
            }).catch(err => console.error(err));
        }
    };
    ...
    const validate = () => {
        if (!formData.numDocumento) { Swal.fire('Errore', 'Inserire il numero bolla', 'error'); return false; }
        if (!formData.dataDocumento) { Swal.fire('Errore', 'Inserire la data bolla', 'error'); return false; }
    ...
    const saveBolla = async () => {
        if (!validate()) return null;

        const parts = formData.dataDocumento.split('-');
        const dtFormatted = `${parts[2]}/${parts[1]}/${parts[0]}`;

        let dtDocumentoFornitoreFormatted = null;
        if (formData.dataDocumentoFornitore) {
            const fParts = formData.dataDocumentoFornitore.split('-');
            dtDocumentoFornitoreFormatted = `${fParts[2]}/${fParts[1]}/${fParts[0]}`;
        }
        ...
        const payload = {
            ...
            dataDocumentoFornitore: dtDocumentoFornitoreFormatted,
        ...
        try {
            const res = await BollaCaricoService.save(payload);
    ...
    const handleSave = async (e) => {
        ...
        const savedId = await saveBolla();
        if (savedId) {
            Swal.fire({
                title: 'Salvato!',
                text: 'Bolla di carico salvata con successo',
                icon: 'success',
                timer: 1500,
                showConfirmButton: false
            }).then(() => navigate('/bollecarico'));
        }
    };

    return (
        <div className="bollacarico-detail-container entity-form-shared">
            <div id="ddt-content-header">
                <div>
                    <h1>{isNew ? 'Nuova' : 'Modifica'} Bolla di carico</h1>
                    <div className="breadcrumb">
                        <span onClick={() => navigate('/bollecarico')}>Elenco Bolle di carico</span> / <span>{isNew ? 'Nuova' : formData.numDocumento}</span>
                    </div>
                </div>
                    ...
                    <div className="tab-content" autoComplete="off">
                        ...
                        <div className={`tab-pane ${activeTab === 'generale' ? 'active' : ''}`}>
                            <div className="tab-padding-wrapper">
                                ...
                                <div className="compact-row">
                                    ...
                                    <div className="compact-col compact-col-sm">
                                        <div className="form-group">
                                            <label>Data Doc. Fornitore</label>
                                            <input
                                                type="date"
                                                className="form-control premium-input"
                                ...
                                {formData.idFornitore && (
                                    <div className="ordini-aperti-panel">
                                        <div className="ordini-aperti-header" onClick={() => setShowOrdiniAperti(!showOrdiniAperti)}>
                                            <span><FaBoxOpen /> Ordini aperti per questo fornitore ({ordiniAperti.length})</span>
                                            {showOrdiniAperti ? <FaChevronUp /> : <FaChevronDown />}
                                        </div>
                                        {showOrdiniAperti && (
                                            <div className="ordini-aperti-list">
                                                {ordiniAperti.length === 0 ? (
                                                    <span style={{ color: '#999', fontSize: '13px' }}>Nessun ordine aperto per questo fornitore.</span>
                                                ) : (
                                                    ordiniAperti.map(o => (
                                                        <div key={o.id} className={`ordine-aperto-item ${isOrdineLinked(o.id) ? 'linked' : ''}`}>
                                                            <input
                                                                type="checkbox"
                                                                checked={isOrdineLinked(o.id)}
                                                                onChange={() => toggleOrdineLink(o)}
                                                            />
                                                            <span><strong>Ordine {o.numDocumento}</strong> del {o.dataDocumento} {o.statoOrdine ? `- ${o.statoOrdine}` : ''}</span>
                                                        </div>
                                                    ))
                                                )}
                                            </div>
                                ...
                                <div className="row mt-4">
                                    ...
                                    <div className="col-md-6">
                                        <div className="premium-card address-card">
                                            <div className="card-header-vibrant">
                                                <span><FaTruck /> Provenienza / Destinazione</span>
                                            </div>
                                            <div className="card-body">
                                                <div className="row mb-4">
                                                    <div className="col-md-12">
                        ...
                        <div className={`tab-pane ${activeTab === 'articoli' ? 'active' : ''}`}>
                            <DocumentRows
                                ...
                                onAddRow={(newRow) => { setProdotti([...prodotti, { ...newRow, scarica: 1 }]); setActiveTab('articoli'); }}
                        ...
                        <footer className="main-box-footer detail-footer">
                            <button type="button" className="btn-premium-cancel" onClick={() => navigate('/bollecarico')}>
                                        ...
                                        {!isNew && (
                                            <>
                                                ...
                                                <button type="button" className="split-btn-item" onClick={async () => {
                                                    const savedId = await saveBolla();
                                                    if (savedId) navigate(`/fatture-fornitore/new?fromBollaCarico=${savedId}`);
                                                }}>
...
export default BollaCaricoDetail;
