...
const RegistrazioneProva = () => {
    ...
    const [tipoAccount, setTipoAccount] = useState(initialPlan);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await api.post('/auth/registrazione-prova', {
                ...
                partitaIva,
                email,
                tipoAccount: parseInt(tipoAccount)
            });

            if (res.data && (res.data.success || res.status === 200)) {
                await Swal.fire({
                    title: 'Prova Gratuita Attivata! 🚀',
                    text: `Abbiamo inviato un'email a ${email}. Clicca sul link nell'email per impostare la tua password ed iniziare subito!`,
                    icon: 'success',
            ...
            <div style={{
                ...
                <div style={{ textAlign: 'center', marginBottom: '28px' }}>
                    ...
                    <p style={{ marginTop: '12px', color: '#64748b', fontSize: '14px', lineHeight: '1.5' }}>
                        Inizia subito i tuoi 90 giorni di prova completa.<br/>
                        <strong style={{ color: '#334155' }}>Nessuna carta di credito richiesta.</strong>
                    </p>
                </div>
                ...
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
                    <div>
                        <label style={{ display: 'block', fontSize: '13px', fontWeight: '700', color: '#334155', marginBottom: '6px' }}>
                            Ragione Sociale / Nome Attività <span style={{ color: '#ef4444' }}>*</span>
                        </label>
                        <input
                            type="text"
                    ...
                    <div>
                        <label style={{ display: 'block', fontSize: '13px', fontWeight: '700', color: '#334155', marginBottom: '6px' }}>
                            Partita IVA / Codice Fiscale <span style={{ color: '#ef4444' }}>*</span>
                        </label>
                        <input
                            type="text"
                    ...
                    <div>
                        ...
                        <input
                            type="email"
                            placeholder="dove riceverai il link di attivazione..."
                            value={email}
                    ...
                    >
                        {loading ? 'Attivazione in corso...' : 'Attiva i tuoi 3 Mesi Gratis 🚀'}
                    </button>
                </form>
